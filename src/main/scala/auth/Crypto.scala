  ...
  def make[F[_]: Sync](passwordSalt: PasswordSalt): F[Crypto] =
    Sync[F]
      .delay {
        ...
        eCipher.cipher.init(Cipher.ENCRYPT_MODE, sKeySpec, iv)
        val dCipher = DecryptCipher(Cipher.getInstance("AES/CBC/PKCS5Padding"))
        dCipher.cipher.init(Cipher.DECRYPT_MODE, sKeySpec, iv)
        (eCipher, dCipher)
      }
      .map { case (ec, dc) =>
        new Crypto {
          def encrypt(password: Password): EncryptedPassword = {
            ...
            val result = new String(base64.encode(ec.cipher.doFinal(bytes)), "UTF-8")
          ...
          def decrypt(password: EncryptedPassword): Password = {
            ...
            val result = new String(dc.cipher.doFinal(bytes), "UTF-8")
            Password(result)
          }
        }
      }
